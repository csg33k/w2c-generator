...
func (r *Repository) CreateSubmission(ctx context.Context, s *domain.Submission) error {
	...
	res, err := r.db.ExecContext(ctx, `
		INSERT INTO submissions (ein,employer_name,addr1,addr2,city,state,zip,zip_ext,
...
func (r *Repository) GetSubmission(ctx context.Context, id int64) (*domain.Submission, error) {
	...
	rows, err := r.db.QueryContext(ctx, `
		SELECT id,submission_id,ssn,original_ssn,first_name,middle_name,last_name,suffix,
		...
		FROM employees WHERE submission_id=?`, id)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (r *Repository) ListSubmissions(ctx context.Context) ([]domain.Submission, error) {
	rows, err := r.db.QueryContext(ctx, `
		SELECT id,ein,employer_name,notes,created_at FROM submissions ORDER BY created_at DESC`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (r *Repository) UpdateSubmission(ctx context.Context, s *domain.Submission) error {
	_, err := r.db.ExecContext(ctx, `
		UPDATE submissions SET ein=?,employer_name=?,addr1=?,addr2=?,city=?,state=?,zip=?,zip_ext=?,
...
func (r *Repository) AddEmployee(ctx context.Context, submissionID int64, e *domain.EmployeeRecord) error {
	...
	e.UpdatedAt = now
	res, err := r.db.ExecContext(ctx, `
		INSERT INTO employees (submission_id,ssn,original_ssn,first_name,middle_name,last_name,suffix,
			addr1,addr2,city,state,zip,zip_ext,
			orig_wages,corr_wages,orig_ss_wages,corr_ss_wages,
			orig_med_wages,corr_med_wages,orig_fed_tax,corr_fed_tax,
			orig_ss_tax,corr_ss_tax,orig_med_tax,corr_med_tax,
...
func (r *Repository) UpdateEmployee(ctx context.Context, e *domain.EmployeeRecord) error {
	...
	_, err := r.db.ExecContext(ctx, `
		UPDATE employees SET ssn=?,original_ssn=?,first_name=?,middle_name=?,last_name=?,suffix=?,
			addr1=?,addr2=?,city=?,state=?,zip=?,zip_ext=?,
			orig_wages=?,corr_wages=?,orig_ss_wages=?,corr_ss_wages=?,
			orig_med_wages=?,corr_med_wages=?,orig_fed_tax=?,corr_fed_tax=?,
			orig_ss_tax=?,corr_ss_tax=?,orig_med_tax=?,corr_med_tax=?,
			updated_at=?
		WHERE id=?`,
		e.SSN, e.OriginalSSN, e.FirstName, e.MiddleName, e.LastName, e.Suffix,
		e.AddressLine1, e.AddressLine2, e.City, e.State, e.ZIP, e.ZIPExtension,
		e.Amounts.OriginalWagesTipsOther, e.Amounts.CorrectWagesTipsOther,
		e.Amounts.OriginalSocialSecurityWages, e.Amounts.CorrectSocialSecurityWages,
		e.Amounts.OriginalMedicareWages, e.Amounts.CorrectMedicareWages,
